...
padZero l1 l2 = (((clone 0 (m - len1)) ++ l1), ((clone 0 (m - len2)) ++ l2))
  where
    len1 = length l1
    len2 = length l2
    m = if len1 > len2 then len1 else len2
...
bigAdd l1 l2     = removeZero res
  where
    ...
    f (a:aa) (x,y) = [(div (x + y + a) 10)] ++  [(mod (x+y+a)  10)] ++ aa
    base         = [0]
...
mulByDigit i n = removeZero res
  where
    res          = foldLeft f base args
    f (a:aa) x = [(div (x * i + a) 10)] ++  [(mod (x* i +a)  10)] ++ aa
    base = [0]
...
bigMul l1 l2 = res
  where
    ...
    f (v, a) x = (v++[0],  (bigAdd a ((mulByDigit x l1) ++ v)))
    base     = ([], [])
    args     = reverse l2
